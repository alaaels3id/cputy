    ...
    <div className="p-8 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white flex items-center gap-2.5">
            ...
            <span>{t('browsersHeaderTitle')}</span>
          </h1>
          <p className="text-xs text-mac-subtext mt-1">
            {t('browsersHeaderDesc')}
          </p>
        </div>
      ...
      {!scanResult || scanResult.items.length === 0 ? (
        <div className="p-12 text-center rounded-3xl cputy-glass border border-mac-border space-y-3">
          ...
          <p className="text-xs text-mac-subtext">{isRTL ? 'بيانات التصفح وكاش ملفات الويب نظيفة تماماً.' : 'Web browser caches, cookies & tracking artifacts are completely clean.'}</p>
        </div>
      ) : (
