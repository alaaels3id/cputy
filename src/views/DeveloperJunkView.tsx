    ...
    <div className="p-8 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white flex items-center gap-2.5">
            ...
            <span>{t('developerJunkHeaderTitle')}</span>
          </h1>
          <p className="text-xs text-mac-subtext mt-1">
            {t('developerJunkHeaderDesc')}
          </p>
        </div>
      ...
      {!scanResult || scanResult.items.length === 0 ? (
        <div className="p-12 text-center rounded-3xl cputy-glass border border-mac-border space-y-3">
          <Terminal className="w-12 h-12 text-emerald-500 dark:text-emerald-400 mx-auto opacity-80" />
          <h3 className="text-sm font-bold text-slate-900 dark:text-white">{t('noJunkFound')}</h3>
          <p className="text-xs text-mac-subtext">{isRTL ? 'مخرجات البناء وحزم التطوير نظيفة ومحسنة.' : 'No residual developer caches or build artifacts found.'}</p>
        </div>
      ) : (
